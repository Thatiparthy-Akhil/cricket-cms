...
const { registerUser, getUserByEmail, getAllUsers } = require('./models/user'); // Include getAllUsers function
const { createArticle, updateArticle, deleteArticle, getAllArticles, likeArticle } = require('./models/article'); // Import article functions
...
app.post('/auth/register', async (req, res) => {
  console.log('Received registration request'); // Debug statement
  const { name, email, password, role } = req.body;
  try {
    const userId = await registerUser(name, email, password, role);
    console.log('User registered successfully:', userId); // Debug statement
    res.status(201).json({ id: userId, name, email, role });
  } catch (error) {
    console.error('Error registering user:', error); // Debug statement
...
app.get('/users', authenticateToken, (req, res) => {
  ...
  pool.query('SELECT * FROM Users LIMIT ? OFFSET ?', [parseInt(limit), offset], (err, results) => {
...
// Existing routes (apply authenticateToken middleware to secure them)
